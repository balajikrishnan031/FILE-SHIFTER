    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-light-bg text-slate-800">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-bold text-zinc-500">Loading Converter Hub...</p>
        </div>
      </div>
    );
  }
        ...
        <main className="flex-1 p-6 md:p-8 flex flex-col gap-8 max-w-5xl">
          ...
          <div className="flex items-center justify-between border-b border-zinc-200 pb-6">
            <div className="flex items-center gap-3">
              <div className={`h-12 w-12 rounded-xl ${catDetails.bg} flex items-center justify-center border border-zinc-200/50 shadow-sm`}>
            ...
            <Link
              href="/dashboard"
              className="inline-flex items-center gap-1 text-xs text-zinc-400 hover:text-zinc-600 transition-colors font-bold"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              {type === "utility" ? (
                ...
                <div className="flex flex-col gap-6">
                  {/* QR Generator Card */}
                  <div className="glass-panel rounded-3xl p-6 border-zinc-200 bg-white/70 flex flex-col gap-4 shadow-sm">
                    <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
                      <Sparkles className="h-4 w-4 text-indigo-500" /> QR Code Generator
                    </h3>
                    ...
                    <div className="flex flex-col gap-3">
                      <input
                        ...
                        className="w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-700 focus:outline-none focus:ring-1 focus:ring-primary shadow-inner"
                      />
                      <button
                        onClick={handleGenerateQR}
                        disabled={!qrText || qrGenerating}
                        className="rounded-xl bg-indigo-650 hover:bg-indigo-600 text-white font-bold py-2.5 text-sm transition-all active:scale-[0.98] disabled:opacity-50 bg-indigo-600"
                      >
                    ...
                    {qrResult && (
                      <div className="flex flex-col items-center gap-3 border-t border-zinc-100 pt-4">
                        <img src={qrResult} alt="Generated QR" className="h-40 w-40 rounded-xl bg-white p-2 border border-zinc-200 shadow-md" />
                        <a
                          href={qrResult}
                          download="qr_code.png"
                          className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-zinc-950 hover:bg-zinc-800 text-white font-bold px-4 py-2 text-xs"
                        >
                          Download QR PNG
                        </a>
                      </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="glass-panel rounded-3xl p-6 border-zinc-200 bg-white/70 flex flex-col gap-4 shadow-sm">
                ...
                <ul className="flex flex-col gap-3">
                  {catDetails.features.map((feat, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-zinc-500 font-semibold leading-normal">
              ...
              <div className="glass-panel rounded-3xl p-6 border-zinc-200 bg-white/70 shadow-sm">
                <h3 className="text-sm font-bold text-zinc-700 uppercase tracking-wider mb-2">
                  Usage Instructions
                </h3>
                <ol className="list-decimal pl-4 text-xs text-zinc-400 font-bold leading-relaxed flex flex-col gap-2">
