    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-light-bg text-slate-800">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-bold text-zinc-500">Loading Shifter Profile...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 md:p-8 flex flex-col gap-6 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* User Profile Card */}
            <div className="glass-panel rounded-3xl p-6 border-zinc-200 bg-white/70 flex flex-col gap-5 items-center text-center">
              <div className="relative">
                <img src={user.avatar} alt={user.name} className="h-24 w-24 rounded-full bg-zinc-100 border-2 border-zinc-200 shadow-md" />
                <span className="absolute bottom-0 right-0 flex h-6 items-center rounded-full bg-gradient-to-r from-primary to-secondary px-2.5 text-[9px] font-bold text-white shadow-md border-2 border-white uppercase tracking-wider">
                  FREE
                </span>
              </div>
              ...
              <div>
                ...
                <div className="inline-flex items-center gap-1 mt-2.5 rounded-full bg-zinc-100 border border-zinc-200/50 px-2.5 py-0.5 text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                  Role: {user.role}
                </div>
              </div>

              <div className="w-full border-t border-zinc-100 pt-4 flex justify-between text-xs text-zinc-400 font-bold">
                <span>Account Created</span>
                <span className="text-zinc-600">{new Date(user.createdAt).toLocaleDateString()}</span>
              </div>
            </div>

            {/* Storage Usage Card */}
            <div className="glass-panel rounded-3xl p-6 border-zinc-200 bg-white/70 flex flex-col justify-between gap-5 md:col-span-2 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={resetUsage}
                  className="text-xs text-zinc-400 hover:text-zinc-600 transition-colors flex items-center gap-1 font-bold"
                >
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-zinc-100 pt-4">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Conversions Count</span>
                  <span className="text-2xl font-extrabold text-slate-800 flex items-center gap-1.5">
                    <Activity className="h-5 w-5 text-primary" /> {user.dailyConversionsCount}
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Plan Tier</span>
          ...
          <div className="glass-panel rounded-3xl p-6 border-zinc-200 bg-white/70 shadow-sm flex flex-col gap-3">
            <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
