    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-light-bg text-slate-800">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-bold text-zinc-500">Verifying Admin Privileges...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-200 pb-6">
            ...
            <div className="flex gap-2">
              <button
                onClick={fetchStats}
                disabled={fetching}
                className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-xs font-bold text-zinc-600 hover:bg-zinc-50 transition-colors shadow-sm"
              >
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold text-white shadow-lg transition-all ${
          ...
          {stats ? (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                {[
                  { title: "Total Conversions", value: stats.totalConversions, desc: "+14% this week", icon: Activity, color: "text-purple-500", bg: "bg-purple-500/10" },
                  { title: "Total Active Users", value: stats.totalUsers, desc: "24 new signups", icon: Users, color: "text-cyan-500", bg: "bg-cyan-500/10" },
                  { title: "Conversions Rate (24h)", value: stats.activeUsers, desc: "Stable throughput", icon: Award, color: "text-emerald-500", bg: "bg-emerald-500/10" },
                  { title: "Temp Cache Space", value: formatSize(stats.totalConversions * 452000), desc: "Sandbox clean logs", icon: Database, color: "text-amber-500", bg: "bg-amber-500/10" }
                ].map((m, i) => {
                  const Icon = m.icon;
                  return (
                    <div key={i} className="glass-panel rounded-2xl p-5 border-zinc-200 bg-white/70 flex flex-col justify-between shadow-sm">
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{m.title}</span>
                      ...
                      <span className="text-[10px] text-zinc-400 mt-2 font-bold">{m.desc}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Daily traffic chart */}
                <div className="glass-panel rounded-3xl p-5 border-zinc-200 bg-white/70 flex flex-col gap-4 shadow-sm">
                  <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                    <TrendingUp className="h-4 w-4 text-purple-500" /> Daily File Conversions
                  </h3>
                  <div className="h-64 w-full text-xs">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={stats.dailyTrends} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        ...
                        <YAxis stroke="#71717a" />
                        <ChartTooltip 
                          contentStyle={{ backgroundColor: "#ffffff", borderColor: "rgba(0,0,0,0.08)", borderRadius: "12px", color: "#0f172a" }}
                        />
                ...
                <div className="glass-panel rounded-3xl p-5 border-zinc-200 bg-white/70 flex flex-col gap-4 shadow-sm">
                  <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                    <Layers className="h-4 w-4 text-cyan-500" /> Popular Output Formats
                  </h3>
                  <div className="h-64 w-full text-xs">
                    <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="flex flex-col gap-3">
                ...
                {stats.recentConversions.length > 0 ? (
                  <div className="glass-panel rounded-2xl overflow-hidden border border-zinc-200 bg-white/70 shadow-sm">
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="border-b border-zinc-200 bg-zinc-50/50 font-bold text-zinc-400 uppercase tracking-wider">
                        ...
                        <tbody>
                          {stats.recentConversions.map((conv, i) => (
                            <tr key={i} className="border-b border-zinc-100 hover:bg-zinc-50/20 text-zinc-600 transition-colors">
                              <td className="p-3 pl-5 font-bold text-zinc-500 truncate max-w-[150px]">
                  ...
                  <div className="glass-panel rounded-2xl p-8 text-center text-xs text-zinc-400 bg-white/70 border border-zinc-200">
